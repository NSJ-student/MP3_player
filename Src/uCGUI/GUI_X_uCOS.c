...
static osMutexId DispSem;
osMutexDef(DispSem);
/*
static osSemaphoreId DispSem;
osSemaphoreDef(DispSem);
*/

//static  OS_EVENT  *EventMbox;
static osSemaphoreId MboxSem;
osSemaphoreDef(MboxSem);

//static  OS_EVENT  *KeySem;
static osSemaphoreId KeySem;
...
{
	DispSem = osMutexCreate(osMutex(DispSem));
//	DispSem = osSemaphoreCreate(osSemaphore(DispSem), 1);
//  DispSem = OSSemCreate(1);             /* 建立一个互斥型信号量 */
	MboxSem = osSemaphoreCreate(osSemaphore(MboxSem), 1);
...
//	osSemaphoreWait(DispSem, osWaitForever);
    osMutexWait(DispSem, osWaitForever);
}

void GUI_X_Unlock(void)
{ 
    osMutexRelease(DispSem);
...
{
	osSemaphoreWait(MboxSem, osWaitForever);
//   uint8_t err;
...
{
	KeySem = osSemaphoreCreate(osSemaphore(KeySem), 0);
...
int GUI_X_WaitKey(void)
{
   ...
   if(KeyPressed==0)
   {
		osSemaphoreWait(KeySem, osWaitForever);
