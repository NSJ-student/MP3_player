...
void Graph_Init(int x_min, int x_max, int y_min, int y_max)
{
  int x, y;
  int x_step = (LCD_XSIZE - 40)/(x_max-x_min);
  int y_step = (YSIZE - 20)/(y_max-y_min);
	...
	for (x = 0; x < (LCD_XSIZE - 20); x += x_step) 
	{
		int xPos = x + 18;
		GUI_DrawVLine(xPos, (LCD_YSIZE - 20), (LCD_YSIZE - 14));
		GUI_DispDecAt(x_min + x/x_step, xPos - 2, (LCD_YSIZE - 9), 1);
	}
  
	for (y = 0; y < YSIZE; y += y_step) 
	{
		int yPos = (LCD_YSIZE - 20) - y;
		GUI_DrawHLine(yPos, 13, 18);
		GUI_GotoXY(1, yPos - 4);
		GUI_DispSDec(y_min + y/y_step, 2);
	}
}

void Draw_Graph(void * pPARAM)
{
  PARAM Param;
  int tDiff, t0, Cnt = 0;
  GUI_RECT Rect = {19, (LCD_YSIZE - 20) - YSIZE, (LCD_XSIZE - 2), (LCD_YSIZE - 21)};
