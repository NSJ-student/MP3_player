...
volatile uint32_t	SPI_Tx_Cplt = 0;
...
void VS1003_BufferZero(void)
{
	uint32_t cnt;
	uint32_t total_loop = 1024/32;
...
void VS1003_WriteData_DMA( uint8_t * buf, uint32_t size)
{		
...
//	HAL_NVIC_EnableIRQ(EXTI9_5_IRQn);
	...
	*/
	...
	HAL_SPI_Transmit_DMA(&hspi3, TxBuffer, SOUND_DMA_SIZE);
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi)
{
	...
	if(hspi == &hspi3)
	{
		...
		AudioSize -= SOUND_DMA_SIZE;
		TxBuffer += SOUND_DMA_SIZE;
		...
		else
		{
			...
			TxBuffer = 0;
			VS1003_Cplt = 1;
			osSignalSet(SoundTaskHandle, SOUND_DMA_CPLT);
		}
	}
}
...
void VS1003_PreparePlay(void)
{
	uint16_t oldmode; 	

	while(  MP3_DREQ ==0 );
	VS1003_WriteReg(SPI_DECODE_TIME,0);// Reset DECODE_TIME
/*
	oldmode = VS1003_ReadReg(SPI_MODE);
	while(  MP3_DREQ ==0 );
	VS1003_WriteReg(SPI_MODE, oldmode|SM_OUTOFWAV);	
	*/
}
/*******************************************************************************
